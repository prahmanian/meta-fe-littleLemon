...
const Nav = ({reservations}) => {
    ...
    const links = navLinks.map(({title, path}) => 
        <li key={title}><NavLink style={({isActive}) => isActive ? activeStyle : undefined} className="Nav-link" aria-label={`Navigate to ${title}`} to={path}  onClick={()=> {setOpen(false)}}> {title} </NavLink></li>
    )
